...
`logs/minutes.csv` 的每行在时间字段后包含 `symbol`、`entropy_dex`、
`hedge_venue`，用于阻止不同市场的数据被误合并分析。一个分钟文件只应对应一个
市场；分析器兼容完全不含这些字段的旧文件，但遇到多个已标识市场会明确失败。
...
- 分钟、信号和交易 CSV 的旧表头与新 schema 不一致时，先关闭读取句柄，再旋转为未占用的 `.old`、`.old.1` 等归档，避免 Windows 重命名失败、不同 schema 混写或覆盖已有归档。
- `--record-only` 启动时立即初始化分钟和信号文件；文件创建、写入或 flush 失败时记录完整异常、设置停止事件并让任务失败，不得继续运行并假装数据已落盘。
- 清理阶段逐个关闭所有交易所。若业务任务和关闭同时失败，传播原业务异常并记录关闭异常；只有没有更早错误时才传播首个关闭错误。
...
- 分钟 CSV 写入市场身份，混合市场分析被拒绝，纯旧 schema 保持可分析。
- 已有归档不会被覆盖，Windows 上旋转前不存在打开的读取句柄。
- record-only 的分钟或信号输出失败都会停止并向调用方传播。
- 分钟最终 flush 失败时仍关闭文件句柄，且次要 close 错误不覆盖 flush 根因。
