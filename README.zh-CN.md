...
引擎不会猜测结果或重发原订单。
新代码只会自动读取 schema v3 的未决执行文件。schema v2 文件会原样保留：处理前
必须人工核对两边订单历史和真实仓位；不可直接删除文件或修改其 schema 版本。
没有未决执行文件的 `--record-only` 升级不受影响。schema v3 的 `settled_at` 是
...
实盘会分别按两边的真实签名账户持有操作系统进程锁。同一台服务器上，只要任一
签名账户已被另一个实盘进程使用，即使交易对不同，第二个进程也会在行情和策略
任务启动前退出。进程退出后锁由操作系统自动释放；仍可能有进程运行时不要删除
或绕过锁文件。不同服务器之间无法共享此锁，必须使用不同的 API 钱包或签名账户。
`--record-only` 不获取此锁。
...
- **崩溃证据与单实例**：动态实盘在发单前写入不含密钥的未决执行日志；重启
  ...
  关闭交易。其余情况要求人工核对。同一台服务器上，只要任一签名账户已被
  另一个实盘进程使用，第二个实盘进程就会被操作系统锁拒绝；仅采集进程不受
  影响。跨服务器运行时必须使用不同的 API 钱包或签名账户。
